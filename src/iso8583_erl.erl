...
load_specification(Filename)->
	iso8583_ascii:load_specification(Filename).
...
get_spec_field(Field,Specification)->
	iso8583_ascii:get_spec_field(Field,Specification).


%% @doc gets the bitmap type
-spec get_bitmap_type(map())->hex|binary.
get_bitmap_type(Specification)->
	iso8583_ascii:get_bitmap_type(Specification).
...
pack(Map_pack,Specification)->
	iso8583_ascii:pack(Map_pack,Specification).
...
unpack(IsoMessage,Specification)-> 
	iso8583_ascii:unpack(IsoMessage,Specification).
...
process_data_element(Bitmap,Index_start,Data_binary,Specification)->
	iso8583_ascii:process_data_element(Bitmap,Index_start,Data_binary,Specification).
...
get_bitmap_subs(Binary_type,Bin_message,Specification)->
	iso8583_ascii:get_bitmap_subs(Binary_type,Bin_message,Specification).
